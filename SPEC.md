...
| home-assistant | **Tailscale-private, never public** (changed 2026-09-12) | Was LAN + Tailscale on purpose, for housemates. Han confirmed on 2026-09-12 that HA is reached **only over Tailscale**, so the LAN exposure no longer buys anything and a security review had flagged `:8123` listening on every interface. Host networking still binds all interfaces — the scoping is done at the host firewall rather than with `server_host`, because a malformed `configuration.yaml` stops HA booting and a firewall rule is reversible. Reversal record: `plan/home-assistant-followups.md` §3. |
